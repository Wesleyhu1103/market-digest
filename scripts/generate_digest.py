...
- Story items in #equities, #tech, #macro, #crypto, #buyside, and #deals use condensed <details class="deal"> dropdowns (NOT <ul><li>). Pattern: <details class="deal"><summary><strong>headline</strong></summary><div class="deal-body">...</div></details>. Equities/tech/macro/crypto/buyside: one <p> in deal-body with the driver/context. Deals: separate <p> blocks labeled Why / Outlook / What it means. After the buyside analyst dropdowns, include one `.buyside-summary` block with the same Bull/Bear/Both toggles and `.bullbear show-bull` structure as `.narrative` (NOT `.buyside-grid` cards).
