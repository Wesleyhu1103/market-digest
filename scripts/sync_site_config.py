...
# ?v=<digits> cache-bust, including archive-safe ../js/ and ../css/ refs.
...
def html_version_targets() -> list[Path]:
    targets = [ROOT / "docs" / rel for rel in HTML_FILES]
    archive_dir = ROOT / "docs" / "archive"
    if archive_dir.exists():
        targets.extend(sorted(archive_dir.glob("*.html")))
    return targets


def sync_html_versions(version: str) -> None:
    for path in html_version_targets():
        if not path.exists():
            continue
        text = path.read_text()
