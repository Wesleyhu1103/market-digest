...
# Matches a local asset reference (src="js/…" or src="../js/…", and the CSS
# equivalents) carrying a ?v=<digits> cache-bust. Leaves CDN URLs and font
...
def html_version_files() -> list[Path]:
    docs = ROOT / "docs"
...
def sync_html_versions(version: str) -> None:
    for path in html_version_files():
        if not path.exists():
            continue
        text = path.read_text()
