...
Single source of truth is docs/site-config.json. Running this:
  ...
  2. rewrites the ?v=<assetVersion> cache-bust on local js/ and css/ tags in
     docs/index.html, docs/admin.html, and archived digest pages so returning
...
def sync_html_versions(version: str) -> None:
    docs = ROOT / "docs"
    html_paths = [docs / rel for rel in HTML_FILES]
    html_paths.extend(sorted((docs / "archive").glob("*.html")))
