...
Single source of truth is docs/site-config.json. Running this:
  ...
  2. rewrites the ?v=<assetVersion> cache-bust on local js/ and css/ tags in
     docs/index.html, docs/admin.html, and archived snapshots so returning
...
# Matches a local asset reference (src="js/…" / href="../js/…" plus css)
...
def sync_html_versions(version: str) -> None:
    html_files = [ROOT / "docs" / rel for rel in HTML_FILES]
    html_files.extend(sorted((ROOT / "docs" / "archive").glob("*.html")))
    for path in html_files:
        ...
        if new_text != text:
            ...
            print(f"Updated {n} asset tags in {rel} -> ?v={version}")
        else:
            print(f"{rel}: {n} asset tags already at ?v={version}")
