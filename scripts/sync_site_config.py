...
Single source of truth is docs/site-config.json. Running this:
  ...
  2. rewrites the ?v=<assetVersion> cache-bust on local js/ and css/ tags in
     docs/index.html, docs/admin.html, and archived digests so returning
...
# Matches a local asset reference (src="js/…" / href="css/…", including
# archive-safe "../" refs) carrying a ?v=<digits> cache-bust. Leaves CDN URLs
# and font query strings untouched.
...
def sync_html_versions(version: str) -> None:
    docs_dir = ROOT / "docs"
    paths = [docs_dir / rel for rel in HTML_FILES]
    paths.extend(sorted((docs_dir / "archive").glob("*.html")))
    for path in paths:
        if not path.exists():
            continue
        text = path.read_text()
