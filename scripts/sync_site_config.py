...
Single source of truth is docs/site-config.json. Running this:
  ...
  2. rewrites the ?v=<assetVersion> cache-bust on local js/ and css/ tags in
     docs/index.html, docs/admin.html, and archive snapshots so returning
     visitors fetch fresh assets after any JS/CSS change.
...
# ?v=<digits> cache-bust. Archive snapshots use ../js and ../css. Leaves CDN
# URLs and font query strings untouched.
...
def sync_html_versions(version: str) -> None:
    paths = [ROOT / "docs" / rel for rel in HTML_FILES]
    paths.extend(sorted((ROOT / "docs" / "archive").glob("*.html")))
    for path in paths:
        if not path.exists():
            continue
        text = path.read_text()
        ...
        if new_text != text:
            ...
            print(f"Updated {n} asset tags in {display} -> ?v={version}")
        else:
            print(f"{display}: {n} asset tags already at ?v={version}")
