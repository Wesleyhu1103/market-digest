...
def _archive_asset_paths(snapshot: str) -> str:
    """Archived pages live one directory below docs/, so shared assets move up."""
    return re.sub(r'((?:src|href)=["\'])(js|css)/', r"\1../\2/", snapshot)
...
def archive_previous_day(current_html: str, today_iso: str) -> None:
    ...
    if not snapshot_path.exists():
        ...
        snapshot = _archive_asset_paths(snapshot)
