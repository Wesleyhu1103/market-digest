...
def archive_safe_asset_paths(html: str) -> str:
    """Make root-level local assets resolve from docs/archive snapshots."""
    html = re.sub(r'(<script\b[^>]*\bsrc=["\'])((?:js)/)', r"\1../\2", html, flags=re.I)
    html = re.sub(r'(<link\b[^>]*\bhref=["\'])((?:css)/)', r"\1../\2", html, flags=re.I)
    return html
