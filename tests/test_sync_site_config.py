...
class SyncSiteConfigTests(unittest.TestCase):
    def test_sync_html_versions_updates_live_admin_and_archive_assets(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (archive / "2026-08-31.html").write_text(
                '<script src="../js/app.js?v=1"></script>'
                '<link rel="stylesheet" href="../css/digest.css?v=1">'
            )

            module.ROOT = root
            module.sync_html_versions("20260901")

            self.assertIn('?v=20260901"', (docs / "index.html").read_text())
            self.assertIn('?v=20260901"', (docs / "admin.html").read_text())
            archive_html = (archive / "2026-08-31.html").read_text()
            self.assertIn('src="../js/app.js?v=20260901"', archive_html)
            self.assertIn('href="../css/digest.css?v=20260901"', archive_html)
