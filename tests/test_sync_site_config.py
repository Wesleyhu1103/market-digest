...
class SyncSiteConfigTests(unittest.TestCase):
    def test_sync_html_versions_updates_archive_snapshots(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            module.sync_html_versions("20260826")

            self.assertIn("?v=20260826", (docs / "index.html").read_text())
            self.assertIn("?v=20260826", (docs / "admin.html").read_text())
            archived = (archive / "2026-08-25.html").read_text()
            self.assertIn('../js/app.js?v=20260826"', archived)
            self.assertIn('../css/digest.css?v=20260826"', archived)
