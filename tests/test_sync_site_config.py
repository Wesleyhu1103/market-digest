...
class SyncSiteConfigTests(unittest.TestCase):
    def test_sync_html_versions_updates_live_admin_and_archives(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            module.ROOT = root
            module.sync_html_versions("20260822")

            self.assertIn("?v=20260822", (docs / "index.html").read_text())
            self.assertIn("?v=20260822", (docs / "admin.html").read_text())
            archive_html = (archive / "2026-08-20.html").read_text()
            self.assertIn('src="../js/config.js?v=20260822"', archive_html)
            self.assertIn('href="../css/digest.css?v=20260822"', archive_html)
