...
class SyncSiteConfigTests(unittest.TestCase):
    def test_sync_html_versions_updates_archive_snapshots(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            module.sync_html_versions("20260918")

            self.assertIn('?v=20260918"', (docs / "index.html").read_text())
            self.assertIn('?v=20260918"', (docs / "admin.html").read_text())
            snapshot = (archive / "2026-09-17.html").read_text()
            self.assertIn('src="../js/app.js?v=20260918"', snapshot)
            self.assertIn('href="../css/digest.css?v=20260918"', snapshot)
