...
function mdSitePath(rel) {
  ...
  const archiveAt = path.indexOf('/archive/');
  if (archiveAt !== -1) return path.slice(0, archiveAt + 1) + clean;
  if (/\.[a-z0-9]+$/i.test(path)) return path.replace(/[^/]+$/, '') + clean;
  return (path.endsWith('/') ? path : path + '/') + clean;
}
