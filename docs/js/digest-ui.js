...
document.querySelectorAll('.quiz .q').forEach((q, qi) => {
  ...
  q.querySelectorAll('.opt').forEach(opt => {
    opt.addEventListener('click', () => {
      ...
      mdPost('/api/quiz', { date: DigestDate.editionIso(), questionIndex: qi, picked: pick, correct: correct, isCorrect: isRight }).catch(function () {});
    });
  });
});
...
function gatherFeedback() {
  const out = { date: DigestDate.editionIso(), submittedAt: new Date().toISOString() };
  document.querySelectorAll('form.fb .opts').forEach(group => {
