...
function digestEditionIso() {
  return (window.DigestDate && DigestDate.editionIso && DigestDate.editionIso()) || new Date().toISOString().slice(0, 10);
}
...
document.querySelectorAll('.quiz .q').forEach((q, qi) => {
  ...
  q.querySelectorAll('.opt').forEach(opt => {
    opt.addEventListener('click', () => {
      ...
      mdPost('/api/quiz', { date: digestEditionIso(), questionIndex: qi, picked: pick, correct: correct, isCorrect: isRight }).catch(function () {});
    });
  });
});
...
function gatherFeedback() {
  const out = { date: digestEditionIso(), submittedAt: new Date().toISOString() };
  document.querySelectorAll('form.fb .opts').forEach(group => {
...
async function submitFeedback(e) {
  ...
  try { localStorage.setItem('marketDigest_feedback_' + data.date, JSON.stringify(data)); } catch(_) {}
  let ok = false;
  try {
