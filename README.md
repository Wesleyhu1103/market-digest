...
| `CRON_SECRET` | **Required for the Vercel cron.** Vercel Cron sends `Authorization: Bearer <CRON_SECRET>` on scheduled calls when this is set; `/api/cron-watchdog` rejects requests when the env var is unset, missing from the request, or mismatched. **Not auto-created — you must set it** (any long random string). |
