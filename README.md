...
| `CRON_SECRET` | **Required.** Vercel Cron sends `Authorization: Bearer <CRON_SECRET>` on scheduled calls. The endpoint rejects requests when this env var is unset or the bearer token does not match, so set any long random string in Production. |
