...
| `CRON_SECRET` | **Required for the Vercel backup to run.** Vercel Cron sends `Authorization: Bearer <CRON_SECRET>` on scheduled calls when this is set; the endpoint rejects all requests if the secret is missing or mismatched. **Not auto-created — you must set it** (any long random string). |
