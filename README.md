...
| `CRON_SECRET` | **Required for the Vercel cron backup.** Vercel Cron sends `Authorization: Bearer <CRON_SECRET>` on scheduled calls; `/api/cron-watchdog` rejects all callers when this variable is unset, so set any long random string in Vercel Production. |
