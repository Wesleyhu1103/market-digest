...
| `CRON_SECRET` | **Required for the Vercel cron backup.** Vercel Cron sends `Authorization: Bearer <CRON_SECRET>` on scheduled calls when this is set; `/api/cron-watchdog` rejects every request when it is unset or the bearer does not match. **Not auto-created — you must set it** (any long random string). |
