...
| `CRON_SECRET` | **Required.** Vercel Cron sends `Authorization: Bearer <CRON_SECRET>` on scheduled calls. **Not auto-created — you must set it** (any long random string). If unset, `/api/cron-watchdog` rejects every request before checking staleness or dispatching GitHub Actions. |
